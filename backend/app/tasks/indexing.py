...
def _parse_documents(kb_id: str = "default") -> list[Document]:
    settings = get_settings()
    data_dir = Path(__file__).resolve().parents[2] / "data"
    if settings.parser == "llamaindex":
        parser = LlamaIndexParserAdapter(data_dir)
    else:
    ...
    if not kb_dir.exists():
        return []
    return parser._parse_kb(kb_dir)
...
def rebuild_index_task(self, kb_id: str = "default"):
    ...
    try:
        ...
        embedding_output = _run_step(
            ...
            task_id,
            "embedding",
            lambda: embedding_stage.execute(EmbeddingInput(chunks=chunks), model_name=runtime_embedding_model),
...
def index_file_task(self, file_id: int):
    ...
    try:
        ...
        documents = _run_step(
            ...
            task_id,
            "parse",
            lambda: parser.parse_file(file_path),
            log_on_complete=lambda docs: f"解析完成：{len(docs)} 个文档",
        )
        if not documents:
        ...
        embedding_output = _run_step(
            ...
            task_id,
            "embedding",
            lambda: embedding_stage.execute(EmbeddingInput(chunks=new_chunks), model_name=runtime_embedding_model),
            log_on_complete=lambda out: f"Embedding 生成完成：{len(out.embeddings)} 条向量",
        )
