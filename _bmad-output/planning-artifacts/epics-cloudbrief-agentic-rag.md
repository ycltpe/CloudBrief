...
### Story 2.1: grade 节点——检索质量评估（工程版 CRAG）

As a 后端开发,
I want 在图中 retrieve 节点之后加入 grade 节点，用 LLM 轻量评估检索结果质量（低 temperature、短 max_tokens、JSON 结构化判定）,
So that 系统能识别低质检索并为后续纠偏提供判定依据，且成本可控、判定可审计。

**Acceptance Criteria:**

**Given** Epic 1 的图骨架与 retrieve 节点输出（RetrievalResult 列表）
**When** 实现 grade 节点：prompt 按 QueryRewriteStage 同模式资产化（独立 stage 文件 + 模板常量），输出 JSON 结构化质量判定
**Then** 节点以低 temperature + 短 max_tokens 调用 ModelClient，判定结果与本次调用的模型/延迟/token 写入图状态
**And** 节点单测（monkeypatch ModelClient）断言：高质量结果→通过判定，低质量/空结果→不通过判定；判定输出只含结构化结论，不含自由文本

### Story 2.2: 改写重试边与跳数控制

As a 企业用户,
I want 检索质量低时系统自动改写我的问题并重试检索（最多 2 跳）,
So that 我不必自己换种问法也能获得更相关的结果。

**Acceptance Criteria:**

**Given** grade 节点的质量判定
**When** 判定为低质时，条件边路由至查询改写节点并重试检索；hop_count 记入图状态
**Then** 重试由代码控制的条件边实现：max_hops=2 硬终止，第二跳起 top_k 减半收敛
**And** 图级测试（固定 LLM 响应序列）断言：低分→重试边、第二次低分→终止边（不再重试）；重试全部发生在生成之前，TTFB 不回退

### Story 2.3: 拒答口径保持与纠偏效果量化验证

As a 运营/管理员,
I want 拒答阈值判定保持与 native 相同的代码硬分支，并在 eval 集上量化纠偏收益,
So that 纠偏逻辑不会击穿拒答口径，且 Phase 2 的价值可度量（退出条件）。

**Acceptance Criteria:**

**Given** 重试终止后的最终检索结果
**When** 结果仍为空或 max_score < refusal_threshold
**Then** 走与 native 同一边逻辑的硬分支拒答（正确性不依赖 LLM 行为）
**And** 图级测试断言：空检索→拒答边、hop 上限后低分→拒答边；同一 eval 集上低质检索用例的回答率/拒答准确率相比 Epic 1 基线的改善可量化；全部 pytest 通过

## Epic 3: 自适应路由与多跳工具化（Phase 3）

plan 节点三档路由（直检/多跳分解/图谱增强），检索工具化（ModelClient 路线 A），tool_trace 落 query_logs。企业用户可问复杂多跳问题（首个 native 做不到的能力）；运营可回放审计每次路由决策。

### Story 3.1: ModelClient 工具调用扩展（路线 A）

As a 后端开发,
I want 扩展 ModelClient.chat 支持 tools / tool_choice 参数并解析 tool_calls 响应,
So that 检索等能力可以作为工具被 plan 节点调用，同时保住现有 provider failover、重试与统一日志，不引入 langchain-openai ToolNode 路线。

**Acceptance Criteria:**

**Given** 现有 ModelClient.chat 为 OpenAI 兼容裸 httpx 客户端（无 tools 参数）
**When** 扩展 chat() 支持 tools / tool_choice 参数与 tool_calls 响应解析
**Then** DashScope 兼容模式端点的 function calling 调通，工具调用请求/响应结构化记录于调用日志
**And** 单测覆盖：tools 参数序列化、tool_calls 解析、无 tools 时行为与现状一致；provider failover / 重试 / 超时行为不变

### Story 3.2: plan 节点三档路由

As a 企业用户,
I want 系统按问题的复杂度自动选择直接检索、多跳分解或图谱增强,
So that 简单问题不被过度处理，复杂问题能被拆解回答。

**Acceptance Criteria:**

**Given** Story 3.1 的工具调用能力与图骨架
**When** 实现 plan 节点（低 temperature + JSON 结构化输出三档路由决策，prompt 按 QueryRewriteStage 同模式资产化）
**Then** 路由决策（direct / multi_hop / graph）与理由写入图状态；图谱档仅在 GraphSchemaStore per-kb 开关 enabled 时可选，关闭时自动降级为 direct
**And** 节点单测（monkeypatch ModelClient）覆盖三档输出与图谱档降级；图级测试断言各档路由边

### Story 3.3: 多跳分解与并行子问题检索

As a 企业用户,
I want 复杂问题被分解为子问题并分别检索后汇总作答,
So that 跨文档的多跳问题也能得到有依据的回答。

**Acceptance Criteria:**

**Given** plan 节点判定为 multi_hop 档
**When** 图将问题分解为子问题，并行检索（同步 Stage 以 asyncio.to_thread 包裹），检索结果汇总后进入生成
**Then** 子问题与各自检索结果记入图状态；max_hops=2 与第二跳 top_k 减半的全局预算仍然生效
**And** 图级测试断言多跳路径的状态流转；并行检索不破坏 RetrievalResult 契约；失败子问题不阻塞整体生成（降级为单跳结果作答或拒答硬分支）

### Story 3.4: tool_trace 编排轨迹落 query_logs

As a 运营/管理员,
I want 每次 agentic 请求的路由决策、工具调用序列、跳数、各跳 max_score、模型/延迟/token 完整落入 query_logs.tool_trace,
So that 每次路由决策可回放审计，成本审视依赖数据而非体感。

**Acceptance Criteria:**

**Given** agentic 模式端到端可运行（含 plan / grade / 多跳）
**When** query_logs 扩展 tool_trace JSON 列，图执行过程将轨迹写入
**Then** 每次 agentic 请求均有完整 tool_trace 记录（覆盖率 100%，可抽样校验）；hop 分布（单跳 / 多跳 / 拒答占比）可统计
**And** native / langchain 路径的 query_logs 行为不变；tool_trace 记录口径与现有日志的敏感信息处理一致

### Story 3.5: 多跳 eval 样本扩充与 Phase 3 验收

**状态：已完成并验收**

As a 运营/管理员,
I want 按通用合成方案扩充 eval 集（30 条两跳 + 10 条直检对照）并跑通双路径评测,
So that agentic 的多跳价值可度量，Phase 3 退出条件达成。

**Acceptance Criteria:**

**Given** 现有 RAGAS eval 集与运行链路
**When** LLM 从现有知识库相关 chunk 对合成需两跳推理的问题，人工抽检后入库（首期 30 条两跳 + 10 条直检对照）
**Then** 同一 eval 集双路径跑分：agentic 的 faithfulness / answer relevancy 不劣于 native，多跳样本通过，拒答口径一致
**And** 评测报告包含：多跳样本通过率、低质用例拒答准确率、P50 延迟增幅 < 30%（单跳）、平均 token/请求增幅 < 2x（多跳样本除外）的对照数据

## Epic 4: 编排状态持久化与中断恢复（Phase 4，触发式）

触发条件出现时（agentic 路径方向无法确认、需中断-检查-恢复辅助决策）引入 checkpointer（SQLite→Redis）与中断恢复。无触发信号则不实施。

### Story 4.1: checkpointer 接入（SQLite 起步）

As a 后端开发,
I want 为图接入 langgraph-checkpoint-sqlite（thread_id == conversation_id），仅持久化编排状态,
So that 跨请求的图状态可恢复，且与会话消息（MySQL）职责分明、无双写真相源。

**Acceptance Criteria:**

**Given** Epic 3 的完整图与现有 ConversationStore（MySQL 会话消息）
**When** 接入 langgraph-checkpoint-sqlite，以 thread_id == conversation_id 关联
**Then** 编排状态（sub_questions、hop_count、tool_trace、中间检索结果）可跨请求恢复；会话消息仍只写 MySQL，无编排状态双写
**And** 节点/图级测试使用 InMemorySaver 注入；SQLite 文件路径走 .env 配置，不硬编码

### Story 4.2: 中断点与恢复执行

As a 运营/管理员,
I want 在高成本操作（如多跳分解执行）前可插入中断点，检查状态快照后再恢复执行,
So that 方向无法确认时我能审计中间状态再决定是否继续。

**Acceptance Criteria:**

**Given** Story 4.1 的 checkpointer
**When** 在图中配置中断点（默认关闭，触发式启用，开关经 SettingMeta 注册、后台可控）
**Then** 中断时完整状态快照可查看；确认恢复后图继续执行，最终结果与不中断执行一致
**And** 图级测试断言中断-恢复路径

### Story 4.3: Redis checkpointer 迁移评估

As a 后端开发,
I want 在需要多 worker 共享编排状态时评估并迁移至 langgraph-checkpoint-redis（复用现有 Redis 6381）,
So that 编排状态可跨进程恢复，且迁移决策基于成熟度证据。

**Acceptance Criteria:**

**Given** SQLite checkpointer 已在单进程验证价值
**When** 评估 langgraph-checkpoint-redis（0.x）成熟度，满足要求则迁移
**Then** 多 worker 下编排状态恢复正确；回退 SQLite 的配置开关保留；现有 Redis 的锁 / Pub-Sub 职责不受影响
**And** 决策（迁移或继续观察）记入 spec memlog
